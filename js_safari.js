...
function updCode(o) {
	...
	var closest = [null, Infinity];
	for(var c = 0; c < document.getElementById("code").children.length; c++) {
		document.getElementById("code").children[c].style.margin = "0px";
	}
	for(var c = document.getElementById("code").children.length-1; c >= 0; c--) {
		//console.log(event.offsetY+" - "+document.getElementById("code").children[c].offsetTop);
		if(event.offsetY < document.getElementById("code").children[c].offsetTop+(document.getElementById("code").children[c].offsetHeight/2)) {
			if(Math.abs((document.getElementById("code").children[c].offsetTop+(document.getElementById("code").children[c].offsetHeight/2))-event.offsetY) < closest[1]) {
				closest = [document.getElementById("code").children[c], document.getElementById("code").children[c].offsetTop];
			}
		}
	}
	drag.targ = null;
...
function addCode(o) {
	...
	if((event.toElement.id == "code") && (drag.e.parentNode.id == "tools")) {
		var clone = drag.e.cloneNode(true);
...
function generateCode() {
	var els = document.getElementById("code").children;
	var code = "";
	var valel = ["right", "left", "up", "down"];
	var translate1 = [
		"right", 
		"left", 
		"up", 
...
var draw = {
	...
	text: function(x, y, text, col, tsize, align, base) {
		...
		console.log(col);
		ca.font = Math.round(tsize*gameScale)+"px Verdana";
		ca.textAlign = align;
	...
	ani: function(x, y, width, height, src) {
		var animation = getAnimation(src);
		var imgWidth = animation.sprite.naturalWidth;
		var imgHeight = animation.sprite.naturalHeight;
		ca.drawImage(
			animation.sprite,
	...
	tile: function(x, y, width, height, src) {
		var tile = getTile(src);
		var imgWidth = tile.tilesheet.naturalWidth;
		var imgHeight = tile.tilesheet.naturalHeight;
		ca.drawImage(
			tile.tilesheet,
			tile.pos.x,
			tile.pos.y,
...
function rest() {
	draw.image(0, 0, window.size.width, window.size.height+window.scale, bg);
}
mwl += 7;
function drawBG() {
	for(var y in game.level.maps.rmap) {
		for(var x in game.level.maps.rmap[y]) {
			if(game.level.maps.rmap[y][x].aniType == "t") {
				draw.tile(x*window.scale, y*window.scale, window.scale, window.scale, game.level.maps.rmap[y][x].src);
				if(game.level.maps.rmap[y][x].src == "floor1") {
					draw.tile(x*window.scale, y*window.scale, window.scale, window.scale, "dirt"+Math.ceil(Math.random()*11));
				}
			} else if(game.level.maps.rmap[y][x].aniType == "a") {
				draw.ani(x*window.scale, y*window.scale, window.scale, window.scale, game.level.maps.rmap[y][x].src);
			}
		}
	}
	if(gameCompleted) {
		var finishText = "Du har klarat ut spevar med ett rekord på "+finalScore+" poäng!";
		draw.filledRect((window.size.width/2)-(draw.textSize(finishText, 16)/2)-5, 0, draw.textSize(finishText, 14)+30, 16+10, "rgba(0,0,0,0.5)");
		draw.text(window.size.width/2, 5, finishText, "rgba(255,255,255,1)", 14, "center", "top");
	}
}
var firstDraw = true;
function drawAll() {
	draw.clear();
	draw.filledRect(0,0,window.size.width,window.size.height,"#222");
	rest();
	for(var y in game.level.maps.rmap) {
		for(var x in game.level.maps.rmap[y]) {
			/*
			...
			if(game.level.maps.imap[y][x].aniType == "t") {
				draw.tile(x*window.scale, y*window.scale, window.scale, window.scale, game.level.maps.imap[y][x].src);
			} else if(game.level.maps.imap[y][x].aniType == "a") {
				draw.ani(x*window.scale, y*window.scale, window.scale, window.scale, game.level.maps.imap[y][x].src);
			}
		}
	}
	for(var y in game.level.maps.rmap) {
		for(var x in game.level.maps.rmap[y]) {
			draw.rect(x*window.scale, y*window.scale, window.scale, window.scale, "rgba(0,0,0,0.1)");
		}
	}
	...
	for(var c in effects) {
		...
		if(typeof effects[c] != "undefined") {
			if(effects[c].type == "tile") {
				...
				var textSize = 30;
				var textWidth = draw.textSize(effects[c].src, textSize);
				draw.filledRect(
					((window.size.width/2)-(textWidth/2))-25, 
					((window.size.height/2)-(textSize/2))-25, 
					textWidth+50, 
					textSize+50, 
					"#000"
				);
				...
				draw.text(window.size.width/2, window.size.height/2, effects[c].src, "#fff", textSize, "center", "middle");
			} else {
	...
	if(firstDraw) {
		...
		ca.clearRect(0, window.size.height, window.size.width, window.scale);
		for(var q = 0; q < lives+tlives; q++) {
			draw.ani(window.scale*q, window.size.height, window.scale, window.scale, "life1");
		}
		var scoreText = "";
		if(game.score == 0) {
		...
		draw.text((window.scale*(lives+tlives))+(window.scale*0.5), window.size.height+(window.scale*0.2), "Poäng: "+scoreText, "#fff", 14);
		if(game.inventory.indexOf("key") != -1) {
			draw.tile(window.size.width-window.scale, window.size.height, window.scale, window.scale, "key");
		}
	}
	if(mouse.distance.down) {
		draw.rect(
			(mouse.distance.from.x*window.scale), 
			(mouse.distance.from.y*window.scale), 
			window.scale, 
			window.scale, 
		"#0f0");
		draw.rect(
			(mouse.distance.to.x*window.scale), 
			(mouse.distance.to.y*window.scale), 
			window.scale, 
			window.scale, 
		"#0f0");
		draw.line(
			((mouse.distance.from.x*window.scale)+(window.scale/2)), 
			((mouse.distance.from.y*window.scale)+(window.scale/2)), 
			((mouse.distance.to.x*window.scale)+(window.scale/2)), 
			((mouse.distance.to.y*window.scale)+(window.scale/2)), 
		"#0f0");
		var tpos = {
			x: (((mouse.distance.from.x+mouse.distance.to.x)/2)*window.scale),
			y: (((mouse.distance.from.y+mouse.distance.to.y)/2)*window.scale)
		};
		//draw.filledRect(tpos.x, tpos.y, window.scale, window.scale, "rgba(0,0,0,0.4)");
		//draw.rect(tpos.x, tpos.y, window.scale, window.scale, "rgba(0,255,0,0.5)");
		draw.filledCircle(tpos.x+(window.scale/2), tpos.y+(window.scale/2), window.scale/2, "rgba(0,0,0,0.4)");
		draw.circle(tpos.x+(window.scale/2), tpos.y+(window.scale/2), window.scale/2, 1, "rgba(0,255,0,0.5)");
		draw.text(
			(((mouse.distance.from.x*window.scale)+(window.scale/2))+((((mouse.distance.to.x*window.scale)+(window.scale/2))-((mouse.distance.from.x*window.scale)+(window.scale/2)))/2)), 
			(((mouse.distance.from.y*window.scale)+(window.scale/2))+((((mouse.distance.to.y*window.scale)+(window.scale/2))-((mouse.distance.from.y*window.scale)+(window.scale/2)))/2)), 
		...
		draw.filledRect((mouse.x)*window.scale, (mouse.y)*window.scale, window.scale, window.scale, "rgba(0,0,0,0.4)");
		draw.rect((mouse.x)*window.scale, (mouse.y)*window.scale, window.scale, window.scale, "rgba(0,255,0,0.5)");
		draw.text((mouse.x+0.5)*window.scale, (mouse.y+0.5)*window.scale, mouse.x+","+mouse.y, "#fff", 10, "center", "middle");
	}
	if((game.startTextOn === true) && (game.level.startMessage !== undefined)) {
		draw.filledRect(0, 0, window.size.width, 100, "#000");
		draw.rect(0, 0, window.size.width, 100, "#fff");
		draw.text(window.size.width/2, 50, game.level.startMessage, "#fff", 14, "center", "middle");
	}
	draw.text(window.size.width/2, window.size.height+6, "Bana "+(level+1)+" av "+levels.length, "#fff", 14, "center", "top");
	if(winAnimation.on == true) {
		winAnimation.f();
		draw.image(winAnimation.x, winAnimation.y, 256*3, 128*3, getImage("win"));
		if(winAnimation.y < (-(128*3))+200) {
			draw.filledRect(0,0,window.size.width,window.size.height,"rgba(0,0,0,"+(1-Math.abs(((winAnimation.y-((-(128*3))))/200)))+")");
		}
...
function resetWin() {
	winAnimation = {
		x: ((window.size.width-(256*3))/2),
		y: window.size.height+5,
		on: false,
		fadeMode: 0,
		f: function() {
			this.y -= 5;
...
var lno = null;
function elog(msg) {
	this.document.querySelector("#errlog").innerText = JSON.stringify(msg);
}
function loaded() {
	if(codemode != "blocks") {
	...
	loadLevel(level);
	resetWin();
	document.getElementById("c").width = window.size.width;
	document.getElementById("c").height = window.size.height+window.scale;
	ca = getCanvas();
	canvas.addEventListener("mousemove", function(e) {
		mouse.x = Math.floor(e.offsetX/window.scale);
		mouse.y = Math.floor(e.offsetY/window.scale);
		if((mouse.x >= 0) && (mouse.x <= window.size.width/window.scale) && (mouse.y >= 0) && (mouse.y < window.size.height/window.scale)) {
			mouse.on = true;
		} else {
			mouse.on = false;
		}
	...
	canvas.addEventListener("mousedown", function(e) {
		...
		mouse.distance.from.x = Math.floor(e.offsetX/window.scale);
		mouse.distance.from.y = Math.floor(e.offsetY/window.scale);
		mouse.distance.to.x = Math.floor(e.offsetX/window.scale);
		mouse.distance.to.y = Math.floor(e.offsetY/window.scale);
	});
	...
	for(var c = 0; c < timgs.length; c++) {
		...
		if(timgs[c].dataset.type == "animation") {
			...
			var size = parseInt(timgs[c].dataset.size);//naturalWidth;
			var names = timgs[c].dataset.names.split(",");
			var c2 = 0;
			for(var y = 0; y < timgs[c].naturalHeight; y += size) {
				for(var x = 0; x < timgs[c].naturalWidth; x += size) {
					if(c2 < names.length) {
						if(names[c2] != "none") {
							tiles.push({
								name: names[c2],
	...
	player: {
		...
		unlock: {
			f: function() {
				...
				var me = funcClass.player.teleport;
				var pos = this.pos[0];
...
var functions = {
	upp: function(c) {	c = (c===undefined)?1:c;	for(var q = 0; q < c; q++) { game.actions.push(funcClass.player.up)	}},
	ner: function(c) {	c = (c===undefined)?1:c;		for(var q = 0; q < c; q++) { game.actions.push(funcClass.player.down)}},
	vänster: function(c) {	c = (c===undefined)?1:c;	for(var q = 0; q < c; q++) { game.actions.push(funcClass.player.left)}},
	höger: function(c) {	c = (c===undefined)?1:c;	for(var q = 0; q < c; q++) { game.actions.push(funcClass.player.right)}},
	ta: function() {game.actions.push(funcClass.player.pickup)},
	läs: function() {game.actions.push(funcClass.player.read)},
	teleportera: function(x, y) {	x = (x===undefined)?false:x; y = (y===undefined)?false:y;	funcClass.player.teleport.pos.push({x: x, y: y}); game.actions.push(funcClass.player.teleport);},
	öppna: function(x, y, pass) {	x = (x===undefined)?false:x; y = (y===undefined)?false:y;pass = (pass===undefined)?false:pass;	funcClass.player.unlock.pos.push({x: x, y: y, pass: pass}); game.actions.push(funcClass.player.unlock);},
	vinn: function() {game.actions.push(funcClass.finish)}
};
mwl ++;
function addEffect(type, x, y, args) {
	x = (x===undefined)?game.level.player.x:x;
	y = (x===undefined)?game.level.player.y:y;
	args = (x===undefined)?["Ingen text"]:args;
	if(type == "teleport1") {
		effects.push({
			x: x,
			y: y,
...
var run = function(cmd) {
	...
	if(ok === true) {
		...
		var tScore = 2500-(game.textScore*5);
		if(tScore < 0) {
			tScore = 0;
		}
		game.score += tScore;
		...
		for(var c in cmds) {
			try {
				...
				if(msg.substr(0, "ReferenceError".length) == "ReferenceError") {
					...
					var ch = msg.substr("SyntaxError: Unexpected token".length+1);
...
function start() {
	...
	if(codemode !== "blocks") {
		var ok = window.run(document.getElementById("inp").value);
	} else {
		var ok = window.run(generateCode());
	}
	updLinecount(document.getElementById("inp"));
}
function gameover() {
	alert("Du har tyvärr förlorat alla extraliv så du får börja om.");
	window.location.reload();
}
